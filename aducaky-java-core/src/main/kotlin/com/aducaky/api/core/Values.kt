...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { AducakyInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw AducakyInvalidDataException("`$name` is not set")
            is JsonNull -> throw AducakyInvalidDataException("`$name` is null")
            else -> throw AducakyInvalidDataException("`$name` is invalid, received $this")
        }
